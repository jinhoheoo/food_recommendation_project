...
#
# form_window = uic.loadUiType('./food_recommendation.ui')[0]
#
# class Exam(QWidget, form_window):
#     def __init__(self):
#         super().__init__()
#         self.setupUi(self)
#         self.Tfidf_matrix = mmread('./models/Tfidf_food_review.mtx').tocsr()
#         with open('./models/tfidf.pickle', 'rb') as f:
#             self.Tfidf = pickle.load(f)
#         self.embedding_model = Word2Vec.load('./models/word2vec_food_review.model')
...
#             recommendation = self.recommendation_by_food_title(key_word)
#         else :
...
#         print(title)
#         recommendation = self.recommendation_by_food_title(title)
#         print('debug01')
#         self.lbl_recommendation.setText(recommendation)
#         print('debug02')
#
...
#
#     def recommendation_by_food_title(self, title):
#         food_idx = self.df_reviews[self.df_reviews['titles'] == title].index[0]
...
#         recfoodList = self.df_reviews.iloc[foodIdx, 0]
...
class Exam(QWidget, form_window):
    ...
    def btn_slot(self):
        ...
        if keyword in self.names:
            recommendation = self.recommendation_by_food_title(keyword)
        else:
    ...
    def recommendation_by_food_title(self, title):
        ...
        food_idx = self.df_reviews[self.df_reviews['names'] == title].index[0]
        print(food_idx)
        cosine_sim = linear_kernel(self.Tfidf_matrix[food_idx], self.Tfidf_matrix)
        recommendation = self.getRecommendation(cosine_sim)
